...
public class NewsController : ControllerBase
{
    ...
    protected virtual IQueryable<News> FilterPredicate(Filter filter, IQueryable<News> entities)
    {
        return entities;
    }
    ...
    public async Task<IActionResult> Get(int? pageIndex, int? pageSize, string? sortField, string? sortType, string? filter)
    {
        try
        {
            ...
            var result = new ListResponse<News>(items, total);
            return Ok(result) ;
        }
        catch (Exception e)
        {
    ...
    public virtual async Task<IActionResult> GetById(long id)
    {
        try
        {
            var model = await _dataContext.News.SingleAsync(m => m.Id == id);
            return Ok(model);
        }
        catch (Exception e)
        {
    ...
    public async Task<IActionResult> Add([FromBody] News model)
    {
        try
        {
            await _dataContext.News.AddAsync(model);
